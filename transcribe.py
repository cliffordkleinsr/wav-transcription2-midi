import numpy as np, librosa, sys, argparse, torch, os
...
from onsets_and_frames.utils import save_pianoroll
from onsets_and_frames.constants import SAMPLE_RATE, HOP_LENGTH, MIN_MIDI, DEFAULT_DEVICE
...
def transcribe_file(model_file, audio_paths, save_path, sequence_length,
    ...
    for i,audio_path in enumerate(tqdm(audio_paths)):
        #print(f'{i+1}/{len(list(audio_paths))}: Processing {audio_path}...', file=sys.stderr)
        audio = load_and_process_audio(audio_path.as_posix(), sequence_length, device)
        ...
        midi_path = os.path.join(save_path, os.path.basename(audio_path) + '.mid')
        save_midi(midi_path, p_est, i_est, v_est)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-n', '--model', type=str, default='model-500000.pt')
    parser.add_argument('-i', '--input', type=Path, default=None, help='Input folder')
    parser.add_argument(
            "-r",
            "--recursive",
            action="store_true",
    ...
    if args.recursive:
        filenames = args.input.rglob("*wav")
    else:
        filenames = args.input.glob("*wav")
    with torch.no_grad():
        #transcribe_file(**vars(parser.parse_args()))
      transcribe_file(args.model, filenames, args.save_path, args.sequence_length, args.onset_threshold, args.frame_threshold, args.device)
